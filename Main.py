...
class Gwin(pyglet.window.Window):
    ...
    def pfire(self, dt):
        ...
        if self.firerate <= 0:
            ...
            self.firerate += 0.09
            music = pyglet.resource.media('assets/pew.wav')
            music.play()
    ...
    def pHealth(self):
        for bpo in self.bullet_list:
            if ((bpo.posx <= self.player2.posx+200) or (bpo.posx <= self.player2.posx-1)) and ((bpo.posy <= self.player2.posy+160) and (bpo.posy >= self.player2.posy-1)):
                self.bullet_list.pop(0)
                self.pth -= 1
    ...
    def pHealth2(self):        #p2
        for bpt in self.bulletEL:
            if ((bpt.posx >= self.player.posx+10000) or (bpt.posx >= self.player.posx)) and ((bpt.posy <= self.player.posy+160) and (bpt.posy >= self.player.posy-1)):
                self.bulletEL.pop(0)
                self.poh -= 1
                print(self.poh," :P1:::::P2: ", self.pth)
        if self.poh == 0:
...
if __name__ == "__main__":
    win = Gwin(1100, 600, "fGame", resizable=True)
